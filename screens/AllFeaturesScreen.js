...
import chatReducers from "../reducers/chatReducers";
import chatReducersactual from "../reducers/chatReducersactual";
import { callKeepSetup, handleConnect,hangupCallIncomingCall,setIncoming,handleCOnnectOnReceive } from './OutGoingCallScreen'
import Star from 'react-native-star-view';
LogBox.ignoreAllLogs();
// import { Video, AVPlaybackStatus } from 'expo-av';
import moment from "moment";
...
import { Text, BottomSheet, ListItem } from "react-native-elements";
import SkeletonPlaceholder from "react-native-skeleton-placeholder";
import { checkNotifications ,request, check, PERMISSIONS} from 'react-native-permissions';
import AsyncStorage from "@react-native-community/async-storage";
import PushNotificationIOS from '@react-native-community/push-notification-ios';
import PushNotification from "react-native-push-notification";
import VoipPushNotification from 'react-native-voip-push-notification';
...
async function handleAPNs(navigation) {
  // VoipPushNotification.requestPermissions(); // --- optional, you can use another library to request permissions
  VoipPushNotification.registerVoipToken(); // --- required
  ...
  async function registerAgain(navigation,notification){
    var callpassword = await AsyncStorage.getItem('callpassword');
    var callid = await AsyncStorage.getItem('aid');
    // handleCOnnectOnReceive()
    handleConnect()
    // handleConnect(callid, callpassword)
    // navigation.naviagte()
    // navigation.navigate('OutGoingCallScreen',{aid:"handle",name:notification.callerName,profilePic:"resp.data.profilePic"})
  }

  VoipPushNotification.addEventListener('notification', async (notification) => {
    console.log(':::::::::::::::RemoteVOIPNotification', notification)
    setIncoming()
    registerAgain(navigation,notification)
    // await axios.get('http://103.27.86.33:3016/trysomething0').then((resp) => {
...
const AllFeaturesScreen = ({ navigation, askName, Userpreferences, chat_roster_main, allMapPosts, token, chat_password, jid_main, userId, refreshAfterPost, distance, sortBy, isConnected, isActive }) => {
  ...
  async function getData(navigation) {
   ...
   if (String(tokenJWT) != "null") {
      ...
      // Chatdispatcher({ type: 'SETMAPPOSTSALL',whatToShow:finalPosts,allMapPosts:finalPosts})
      Chatdispatcher({ type: 'SETTOKEN', token: null, name: 'Loading', userId: null, jid_main: jid_main, chat_password: chat_password })
      // Chatdispatcher({type:'SETPROFILEPIC',profilePic:'https://static-content.spaarksweb.com/images/userprofile.png'})
    }
  ...
  async function checkPostcreated() {
    setInterval(async () => {
      var a = await AsyncStorage.getItem('isAboutToCreate')
      var b = await AsyncStorage.getItem('isAboutToCreateTime')
     
      if (String(a) == "true") {
        if (JSON.parse(b) < Date.now()) {
          await AsyncStorage.setItem('isAboutToCreate', String(false))
          // alert('Why Dint you create post')
          await PushNotificationIOS.addNotificationRequest({
            id: String(Date.now()),
            title: 'Tap to Resume Creating Spaark',
            subtitle: '',
            body: 'Post a Spaarks now and attract audience',
            userInfo: {
              featureName: '',
              apnType: 'NewInfoStepper',
              local: true,
              content: 'item.name',
  ...
  async function showIncomingCall() {
    // alert('Incoming')
    navigation.navigate('IncomingCallScreen', { aid: 'aid', name: 'name', profilePic: 'profilePic' })
  }
  ...
  const checkPermission = () => {
    ...
    check(PERMISSIONS.IOS.MICROPHONE).then(response => {
      console.log('MICROMICRO',response);
    });
...
// }, 5000)
      ...
      onNotification: async function (notification) {
        ...
        if (notification.userInteraction) {
          ...
          if (notification.data.apnType == 'PostSpecificScreen') {
            ...
            var jwt = await AsyncStorage.getItem('token');
            // console.log(GLOBAL.BASE_URL + notification.data.featureName + '/post/' + notification.data.postId)
            await axios.get(GLOBAL.BASE_URL + notification.data.featureName + '/post/' + notification.data.postId,
            ...
            navigation.navigate("GreetingsScreen");
          } else if (notification.data.apnType == 'NewInfoStepper') {


            var content = await AsyncStorage.getItem('content')
            var question = await AsyncStorage.getItem('question')
            var featureName = await AsyncStorage.getItem('featureName')
            var questionNo = await AsyncStorage.getItem('questionNo')
            var category = await AsyncStorage.getItem('category')
            var subCategory = await AsyncStorage.getItem('subCategory')
            var isCallOn = await AsyncStorage.getItem('isCallOn')
            var categoryId = await AsyncStorage.getItem('categoryId')
            var subCategoryId = await AsyncStorage.getItem('subCategoryId')
            var isTag = await AsyncStorage.getItem('isTag')
            var skippedCat = await AsyncStorage.getItem('skippedCat')
            var skippedSubCat = await AsyncStorage.getItem('skippedSubCat')
            ...
            navigation.navigate("NewInfoStepperScreen", {
              ...
              isTag: Boolean(isTag),
              skippedCat: false,
              skippedSubCat: Boolean(skippedSubCat),
              mediaP: [],
              mediaV: []
            })
            // navigation.navigate('NewInfoStepperScreen2',{question:question,questionNo:questionNo,category:category,subCategory:subCategory,categoryId:categoryId,subCategoryId:subCategoryId,images:[],video:[],featureName:featureName,isCallOn:isCallOn,isChatOn:isChatOn,content:content.trim(),isPhoneOn:!isPhoneOn})
          }else if(notification.data.apnType == 'CallLogsScreen'){
                   
            navigation.navigate("CallLogsScreen")
          }else if(notification.data.apnType == 'CallEnded'){      
            // hangupCallIncomingCall()
          }else if(notification.data.apnType == 'RewardsScreen'){      
  ...
  const getDataWithinOnRefresh = async () => {
    if (isConnected) {
      ...
      if (String(jwt) != "null") {
        ...
        // alert(GLOBAL.BASE_URL + "user/post/static/within")
        // alert('not llloggedin')
  ...
  async function onSearch() {
    ...
    // registerCalls()
    // showCount()
    // navigation.navigate('SearchScreenAll')
    // navigation.navigate('GreetingsScreen')
    ...
    <SafeAreaView>
      ...
      >
        ...
        <View style={{ backgroundColor: '#f2f2f2' }}>
          <View style={{ flexDirection: 'row' }}>
            {/* <SpaarksHeading within={true}/> */}
            <SpaarksHeading within={true} showing={sortedDistance} setOffsetWithin={setOffsetWithin} setSortByString={setSortByString} setsortedDistance={setsortedDistance} setOffsetBeyond={setOffsetBeyond} setDataSourceWithin={setDataSourceWithin} setDataSourceBeyond={setDataSourceBeyond} getDataWithinOnRefresh={getDataWithinOnRefresh} getDataBeyondOnRefresh={getDataBeyondOnRefresh} featureName={"all"} getDataWithinOnRefreshSort={getDataWithinOnRefreshSort} getDataBeyondOnRefreshSort={getDataBeyondOnRefreshSort} />
            {/* {
      true ?
        <View style={{ flexDirection: "row" }}>
          <Text style={{ fontWeight: "bold", margin: 0, fontSize: 16, padding: 10 }}>{'  '}{I18n.t('Spaarks')} within <Text style={{ color: '#6FA4E9' }}>{distance.toFixed(0)}Km</Text></Text>
        ...
        <View style={{ backgroundColor: "#f2f2f2" }}>
          {
            dataSourceWithin.length == 0?
               <Text style={{textAlign:'center'}}>No Spaarks</Text>
                :
<></>
          }
          <FlatList
        ...
        <View style={{ backgroundColor: '#f2f2f2', paddingTop: 0 }}>
          <View style={{ flexDirection: 'row' }}>
            ...
            {/* <SpaarksHeading within={false} setOffsetWithin={setOffsetWithin} setSortByString={setSortByString} setsortedDistance={setsortedDistance} setOffsetBeyond={setOffsetBeyond} setDataSourceWithin={setDataSourceWithin} setDataSourceBeyond={setDataSourceBeyond} getDataWithinOnRefresh={getDataWithinOnRefresh} getDataBeyondOnRefresh={getDataBeyondOnRefresh} featureName={"all"} getDataWithinOnRefreshSort={getDataWithinOnRefreshSort} getDataBeyondOnRefreshSort={getDataBeyondOnRefreshSort}/> */}
