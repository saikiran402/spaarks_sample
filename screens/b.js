...
const HomeScreen = ({navigation,route,Userpreferences,selectedPreference}) => {
  const theme = useTheme();
  ...
  return (
    <View style={styles.container}>
     ...
     <Text>Showing you nearby spaarks</Text>
      ...
      >
         <MapView.Marker key={'me'} ref={ref => { this.mark = ref; }} coordinate={ChatState.coordinate} onPress={(e) => onMarkerPress(e)}  identifier="Marker1" pinColor={"blue"} title={"Your Location"} identifier={"Your Location"} />
 ...
 />
        {ChatState.posts.map((marker, index) => {
          ...
          return (
            ...
            //     <Animated.Image
            //       // source={require('../assets/map_marker.png')}
            //       source={{uri:'https://static.thenounproject.com/png/9868-200.png'}}
            //       style={[styles.marker, scaleStyle]}
            //       resizeMode="cover"
            //     />
            //   </Animated.View>
      ...
      <View style={styles.searchBox}>
        ...
        <View style={{ alignItems: 'center' }}>
          <View style={{ flexDirection: 'row',alignItems:'center' }}>
            ...
            <ModalDropdown options={['1Km', '2Km','3Km','4Km','5Km']} keyboardShouldPersistTaps={"always"} textStyle={{fontSize:15,color:'#'}} defaultValue={distance+I18n.t("Km")} dropdownStyle={{width:50,left:Dimensions.get('window').width-60,textAlign:'center',justifyContent:'center',alignItems:'center'}} onSelect={(val)=>sortByRadius(val+1)}/>
          </View>
          <View>
      ...
      </View>
      {
        ...
        <View style={{flex:0,position:'absolute',top:70}}>
        {/* <ScrollView
          horizontal={true}
          scrollEventThrottle={1}
          showsHorizontalScrollIndicator={false}
         ...
         <FlatList
                                            ...
                                            renderItem={({ item }) =>( 
                                              <>
                                              {
                                                ...
                                                <Text style={styles.chipTexts,{color:selectedPreferences == item.subCategory?'#fff':'#000',padding:8}}>{item.subCategory}</Text>
                                                </View>
...
<View
              ...
              <View style={{ flexDirection: 'row', margin: 5, marginTop: 5, alignSelf: 'flex-start', }}>
                ...
                <View>
                  <Text style={{ justifyContent: 'center', textAlign: 'center', color: '#6FA4E9', fontWeight: 'bold', marginBottom: 5, height: 30, marginLeft: 70 }}>{item.item.myshares && item.item.myshares.length} {I18n.t("Shares")}</Text>
                </View>
              </View>
        ...
        :
        <>
        <View style={{backgroundColor:'#fff',width:'200%',padding:5,marginLeft:-30,justifyContent:'center',marginTop:30}}>
          <Text style={{textAlign:'center',marginLeft:-10}}>{("No Posts found nearby in")} {subCategory}</Text>
        </View>
        </>
        }
      </Animated.ScrollView>
